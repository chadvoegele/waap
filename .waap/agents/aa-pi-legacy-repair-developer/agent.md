...
This direct-Pi run implements `tt-implement-legacy-waap-state-migration-repair` in the supplied shared review worktree on `aa-pi-central-state-context-developer`, which backs PR #3. The launcher owns waap state. Do not create another branch or PR, merge main, edit `.waap`, invoke waap, or manage worktrees. Implement only this ticket, use disposable repositories, run all validations, commit implementation changes with agent and ticket IDs, and push the checked-out shared branch to update PR #3. Leave the ticket incomplete pending review.
