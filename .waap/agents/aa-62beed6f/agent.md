...
# Purpose
Implement code for `tt-make-agent-state-commit-writes-idempotent-skip-no-op-statu-cbb0`.

# Instructions
Implement the functionality described in `/.waap/tickets/tt-make-agent-state-commit-writes-idempotent-skip-no-op-statu-cbb0/ticket.md`. Inspect the referenced source before implementing; make the smallest correct change.
...
Before you start: `waap ticket update --ticket-id tt-make-agent-state-commit-writes-idempotent-skip-no-op-statu-cbb0 --set-status in-progress`.
...
`waap ticket update --ticket-id tt-make-agent-state-commit-writes-idempotent-skip-no-op-statu-cbb0 --set-status completed`
