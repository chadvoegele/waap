...
You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-prevent-commands-from-invalidating-waap-state/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-prevent-commands-from-invalidating-waap-state/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-prevent-commands-from-invalidating-waap-state --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-prevent-commands-from-invalidating-waap-state --set-status completed
```
...
Include both `aa-prevent-invalid-waap-state` and `tt-prevent-commands-from-invalidating-waap-state` in the commit message.
