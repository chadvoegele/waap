...
You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-move-agent-system-adapters-under-srcagent/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-move-agent-system-adapters-under-srcagent/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-move-agent-system-adapters-under-srcagent --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-move-agent-system-adapters-under-srcagent --set-status completed
```
...
Include both `aa-move-agent-system-adapters` and `tt-move-agent-system-adapters-under-srcagent` in the commit message.
