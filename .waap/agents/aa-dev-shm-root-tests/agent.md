...
You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-use-dev-shm-for-git-isolated-root-tests/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-use-dev-shm-for-git-isolated-root-tests/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-use-dev-shm-for-git-isolated-root-tests --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-use-dev-shm-for-git-isolated-root-tests --set-status completed
```
...
Include both `aa-dev-shm-root-tests` and `tt-use-dev-shm-for-git-isolated-root-tests` in the commit message.
