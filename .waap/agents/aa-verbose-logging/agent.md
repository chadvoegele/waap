...
You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-add-global-verbose-logging-mode/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-add-global-verbose-logging-mode/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-add-global-verbose-logging-mode --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-add-global-verbose-logging-mode --set-status completed
```
...
Include both `aa-verbose-logging` and `tt-add-global-verbose-logging-mode` in the commit message.
