...
This direct-Pi run implements `tt-repair-moved-waap-state-worktrees-and-upstream-metadata` in the shared PR #3 worktree on `aa-pi-central-state-context-developer`. The launcher owns waap state. Do not create another branch or PR, merge main, edit `.waap`, invoke waap, or manage worktrees. Use disposable repositories. Run all validations, commit implementation changes with agent and ticket IDs, and push the current shared branch. Leave the ticket incomplete pending review.
