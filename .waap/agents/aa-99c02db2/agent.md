...
status = "completed"
session_id = "019f37fb-73bd-7dd1-882c-cd1a7ed8e6dc"
system = "codex"
+++

# Purpose

You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-isolate-tests-from-user-git-configuration/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-isolate-tests-from-user-git-configuration/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-isolate-tests-from-user-git-configuration --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-isolate-tests-from-user-git-configuration --set-status completed
```
...
Include both `aa-99c02db2` and `tt-isolate-tests-from-user-git-configuration` in the commit message.
