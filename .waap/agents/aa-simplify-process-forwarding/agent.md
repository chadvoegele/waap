...
You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-simplify-attached-process-output-forwarding/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-simplify-attached-process-output-forwarding/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-simplify-attached-process-output-forwarding --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-simplify-attached-process-output-forwarding --set-status completed
```
...
Include both `aa-simplify-process-forwarding` and `tt-simplify-attached-process-output-forwarding` in the commit message.
