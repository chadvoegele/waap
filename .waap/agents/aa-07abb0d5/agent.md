+++
creation_date = 2026-07-06T15:10:26Z
status = "running"
session_id = "019f37fb-63ef-7a51-869d-dbd4b18e6f76"
system = "codex"
+++

# Purpose

You are a waap developer agent. Your role is to implement the functionality described in `.waap/tickets/tt-unify-ticket-and-agent-name-slug-ids/ticket.md`.

# Workflow

1. Keep a work log of what you did (see Work Log below).
2. Read `.waap/tickets/tt-unify-ticket-and-agent-name-slug-ids/ticket.md` and the referenced specifications.
...
```sh
waap ticket update --ticket-id tt-unify-ticket-and-agent-name-slug-ids --set-status in-progress
```
...
```sh
waap ticket update --ticket-id tt-unify-ticket-and-agent-name-slug-ids --set-status completed
```
...
Include both `aa-07abb0d5` and `tt-unify-ticket-and-agent-name-slug-ids` in the commit message.
