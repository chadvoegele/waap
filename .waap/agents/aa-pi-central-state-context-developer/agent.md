...
This run is launched directly with Pi, not `waap agent run`. The launcher will
create the isolated worktree, mark the ticket and agent running, and handle
agent metadata after you exit.

Your assigned ticket is
`tt-introduce-central-waap-project-context-resolution`.

Work only in the worktree supplied in the launch prompt. Do not edit `.waap/`,
do not use the `waap` CLI, and do not merge into `main`; those actions must stay
in the canonical checkout while waap is still using legacy state. Do not create
or remove worktrees yourself. Keep notes in your final response instead of a
`.waap` work log.

Replace the role's integration step with this review workflow:

1. Implement, test, format, lint, and build the ticket in your assigned branch.
2. Rebase the branch on current `origin/main` if necessary.
3. Commit only the implementation changes, with both your agent ID and ticket
   ID in the commit subject.
4. Push the branch to `origin`.
5. Create a pull request targeting `main` using:
   `GH_TOKEN="$(nopass_pass.sh p/github.com/chadvoegele/admin_token)" gh pr create --base main --head <your-branch> --fill`
   Never print, log, or otherwise expose the token. If PR creation fails after
   a successful push, report the exact non-secret failure and leave the branch
   pushed for the launcher to create the PR.
6. Do not mark the ticket completed: it awaits human PR review.

The change is a foundational, behavior-changing modification to waap. Preserve
existing behavior until later activation tickets, use only disposable test
repositories for central-state behavior, and avoid touching this repository's
legacy `.waap` state.
