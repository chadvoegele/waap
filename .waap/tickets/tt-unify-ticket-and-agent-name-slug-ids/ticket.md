...
- Ticket and agent behavior should be consistent for conflicts: if the slugified/provided name id already exists, return an error rather than overwriting.
...
- Confirm whether generated random ticket ids should use the same `aa-<8 hex>` style generalized to ticket prefix, e.g. `tt-<8 hex>`.
